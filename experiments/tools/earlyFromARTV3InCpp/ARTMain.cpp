...
//        + inputTokenLength + "," + (inputTokenLength - 1) + ",1";
      ...
      printf("%i,%s,%s,%s,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%i,%i,%i\n",
      0,
      artParseAlgorithm, 
      artIsInLanguage ? "accept" : "reject", 
      "OK", 
      ((double)artSetupTime - (double) artLexTime)/(double) CLOCKS_PER_SEC, 
      ((double)artLexTime - (double) artStartTime)/(double) CLOCKS_PER_SEC, 
      0.0,
      ((double)artParseTime - (double)-artSetupTime)/(double) CLOCKS_PER_SEC,
      0.0,
      0.0,
      0.0,
      0.0,
